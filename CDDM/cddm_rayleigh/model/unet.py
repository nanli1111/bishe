...
from dataset.dataset import get_signal_shape
...
class UNet(nn.Module):
    def __init__(self,
                 ...
                 pe_dim=10,
                 residual=False,
                 in_channels=None) -> None:     # ✅ 新增 in_channels
        super().__init__()
        
        C, H = get_signal_shape()  # 获取信号的形状（这里的 C=2, H=48，是“物理信号”通道数）
        self.signal_channels = C   # ✅ 真正要预测 eps 的通道数（=2）

        # 如果不指定 in_channels，就默认 = C（兼容旧代码）
        if in_channels is None:
            in_channels = C
        self.in_channels = in_channels

        # ✅ 输入投影：把 in_channels（例如 4）压到 signal_channels（2）
        self.input_proj = nn.Conv1d(in_channels, self.signal_channels, kernel_size=1)
        ...
        # ✅ 这里用的是“内部工作通道数”，从 2 开始
        prev_channel = self.signal_channels
        ...
        # ✅ 输出还是 2 通道（C），对应 eps 的维度
        self.conv_out = nn.Conv1d(prev_channel, self.signal_channels, 3, 1, 1)

    def forward(self, x, t):
        # x: [B, in_channels, H]，例如 in_channels=4 (2 路 x_t + 2 路 h_r)
        # ✅ 先投影到 2 通道
        x = self.input_proj(x)     # [B, signal_channels=2, H]
...
unet_1_cfg = {'type': 'UNet', 'channels': [10, 20, 40, 80], 'pe_dim': 128,'in_channels': 4}        # ✅ 2 路 x_t + 2 路 h_r
unet_res_cfg = {
    'type': 'UNet',
    'channels': [10, 20, 40, 80],
    'pe_dim': 128,

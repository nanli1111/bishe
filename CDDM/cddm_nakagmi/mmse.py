...
if __name__ == "__main__":
    ...
    csv_dir = 'CDDM/cddm_nakagmi/ber_result'
    os.makedirs(csv_dir, exist_ok=True)
    csv_path = os.path.join(csv_dir, 'baseline_ber.csv')

    with open(csv_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['snr_db', 'baseline_ber'])
        for snr, ber in zip(snr_range, baseline_bers):
            writer.writerow([float(snr), float(ber)])

    print(f"✅ BER 结果已保存到: {csv_path}")


    # 绘制并保存BER曲线
    plot_ber_curve(baseline_bers, snr_range, save_path=f'CDDM/cddm_nakagmi/ber_result/baseline.png')

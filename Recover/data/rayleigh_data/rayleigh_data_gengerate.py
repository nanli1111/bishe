...
class UAVBaseDatasetGenerator:
    ...
    def apply_rayleigh_channel(self, baseband_stream):
        """
        单径平坦瑞利衰落：每个符号一个 h，符号内 16 个采样点共用同一个 h。
        """
        # 计算“符号数”，向上取整，保证长度够覆盖整个 baseband_stream
        num_symbols = int(np.ceil(len(baseband_stream) / self.SAMPLES_PER_SYMBOL))
        
        # 生成每个符号对应的瑞利信道系数 h[k]
        h_sequence_per_symbol = self._generate_rayleigh_h(num_symbols)   # 形状: [num_symbols]
        
        # 将每个符号的 h 扩展到采样级：每个符号重复 SAMPLES_PER_SYMBOL 次
        upsampled_h = np.repeat(h_sequence_per_symbol, self.SAMPLES_PER_SYMBOL)

        # 应用衰落信道：逐采样乘以对应的 h
        faded_stream = (baseband_stream * upsampled_h).astype(np.complex64)

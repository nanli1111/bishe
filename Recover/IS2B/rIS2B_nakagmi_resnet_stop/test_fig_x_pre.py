...
def visualize_comparison_overlay(model, is2b, tx_clean, rx_faded, h_np, snr_list, sps=16, 
    ...
    for snr_db in snr_list:
        ...
        with torch.no_grad():
            ...
            # B. Rectified Flow
            x_rf = is2b.sample(y=y_tensor_noisy, h=h_tensor, guidance_scale=1.0)
        ...
        axs[0, 2].plot(x_rf_np[0, 0, :], label='Rectified Flow', color='green', alpha=0.9)
        ...
        axs[1, 2].plot(x_rf_np[0, 1, :], label='Rectified Flow', color='green', alpha=0.9)
        axs[1, 2].set_title("Result Q")

        plt.suptitle(f"Waveform Comparison (TimeResNet1D) - SNR: {snr_db}dB", fontsize=16)
        plt.tight_layout()
...
def visualize_constellation_comparison(model, is2b, tx_clean, rx_faded, h_np, snr_list, sps=16, 
    ...
    for snr_db in snr_list:
        ...
        with torch.no_grad():
            ...
            # 3. Rectified Flow
            # 为了节省时间，星座图可视化时 guidance_scale 设为 1.0 (或者和你测试一致)
            x_rf = is2b.sample(y=y_tensor_noisy, h=h_tensor, guidance_scale=1.0)
        ...
        # Subplot 1: Noisy Input (Raw Received)
        ...
        # Subplot 3: Rectified Flow Output
        axs[2].scatter(rf_I, rf_Q, s=s_val, c='green', alpha=alpha_val, label='Rectified Flow')
        axs[2].scatter(gt_I, gt_Q, s=s_val, c='black', alpha=0.1)
        axs[2].set_title("Rectified Flow Output")
        axs[2].set_xlim(-lim, lim); axs[2].set_ylim(-lim, lim)
        axs[2].grid(True, linestyle='--', alpha=0.5)

        # Subplot 4: Overlay (One-Step vs RF) - 看看谁更聚拢
        axs[3].scatter(os_I, os_Q, s=s_val, c='blue', alpha=0.3, label='One-Step')
        axs[3].scatter(rf_I, rf_Q, s=s_val, c='green', alpha=0.3, label='RF')
        axs[3].set_title("Overlay Comparison")
        axs[3].set_xlim(-lim, lim); axs[3].set_ylim(-lim, lim)
        axs[3].legend(loc='upper right')
        axs[3].grid(True, linestyle='--', alpha=0.5)

        plt.suptitle(f"Constellation Diagram Comparison - SNR: {snr_db}dB", fontsize=16)
        plt.tight_layout()
...
if __name__ == "__main__":
    ...
    # === 路径配置 ===
    ckpt_path = fr'IS2B/rIS2B_nakagmi_all_h_resnet_stop/results/best_model_IS2B_resnet.pth'
    vis_wav_dir = 'IS2B/rIS2B_nakagmi_all_h_resnet_stop/vis_waveforms'
    vis_con_dir = 'IS2B/rIS2B_nakagmi_all_h_resnet_stop/vis_constellations'

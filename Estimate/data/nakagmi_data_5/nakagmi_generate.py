...
class UAVBaseDatasetGenerator:
    ...
    def pulse_shaping(self, symbols):
        ...
        if actual_peak > 1e-9: baseband_stream /= 1
    ...
    def apply_pa_nonlinearity(self, signal_stream):
        # Rapp Model 参数
        smoothness_factor = 3.0  # p因子，控制饱和的弯曲程度
        saturation_level = 2.0   # 饱和电平，超过这个值就被削顶
        
        magnitude = np.abs(signal_stream)
        phase = np.divide(signal_stream, magnitude, out=np.ones_like(signal_stream), where=magnitude!=0)
        
        # Rapp 公式
        new_magnitude = magnitude / (1 + (magnitude / saturation_level)**(2 * smoothness_factor))**(1 / (2 * smoothness_factor))
        
        return (new_magnitude * phase).astype(np.complex64)

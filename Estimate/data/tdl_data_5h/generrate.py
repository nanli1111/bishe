...
class UAVBaseDatasetGenerator:
    """
    生成一个“基础”数据集，用于 CNN / Diffusion 任务。
    修改点：
    - True H: 保存 5 条路径 (5 taps) 的 h 轨迹
    - Est  H: 仍然稀疏（窗口内只保留中心点，其它置 0），但对每条路径都保留中心点
    """
    def __init__(self):
        # --- 基本参数 ---
        ...
        self.WINDOW_SIZE_IN_SYMBOLS = 11  # 每个样本窗口大小（符号数）
        ...
        self.OUTPUT_DIR = r"Estimate/data/tdl_data_5h"
    ...
    def print_summary(self):
        ...
        print(f"True H: 5 条路径 × 窗口轨迹 (Dense)")
        print(f"Est  H: 5 条路径 × 窗口稀疏 (仅中心点非零)")
    ...
    # =========================================================
    # ✅ 修改点 1：返回 5 条路径的 h_taps，而不是 main_path_h
    # =========================================================
    def apply_tdl_channel(self, baseband_stream, k_factor_db):
        ...
        tap_powers = 10**(self.TDL_D_POWERS_DB / 10.0)
        tap_powers /= np.sum(tap_powers)

        num_taps = len(tap_indices)

        output_signal = np.zeros(num_samples, dtype=np.complex64)
        tap_h = np.zeros((num_taps, num_samples), dtype=np.complex64)  # [5, num_samples]
        ...
        for i in range(num_taps):
            ...
            tap_h[i, :] = h_curr.astype(np.complex64)
        ...
        return output_signal, tap_h  # ✅ 返回 5 条路径
    ...
    def generate_channel_estimate(self, true_h_stream, nmse_db):
        ...
        # true_h_stream: [num_taps, num_samples]
        h_power = np.mean(np.abs(true_h_stream)**2, axis=1, keepdims=True)  # [num_taps,1]
        error_std = np.sqrt(h_power * nmse_linear / 2)                      # [num_taps,1]
        error_noise = error_std * (np.random.randn(*true_h_stream.shape) + 1j * np.random.randn(*true_h_stream.shape))
        return (true_h_stream + error_noise).astype(np.complex64)

    # =========================================================
    # ✅ 修改点 3：窗口切片时保存 [5, S] 的 true_h 与稀疏 est_h
    # =========================================================
    def sliding_window_slicing(self, ideal_baseband_stream, impaired_stream, filter_delay, labels,
                               true_h_stream, estimated_h_stream):
        """
        true_h_stream:      [5, num_samples]  (5 taps)
        estimated_h_stream: [5, num_samples]  (5 taps)
        返回：
          h_true_vectors: [N,5,S]
          h_est_vectors : [N,5,S] (只中心非零)
        """
        ...
        for i, peak_pos in enumerate(symbol_peak_positions):
            ...
            if (sample_start >= 0 and sample_end < len(impaired_stream) and
                ...
                indices = symbol_peak_positions[sym_start_idx:sym_end_idx]  # len = S

                # [5, S]
                h_true_dense = true_h_stream[:, indices]
                h_true_vectors.append(h_true_dense)

                h_est_chunk = estimated_h_stream[:, indices].copy()          # [5,S]

                # 稀疏导频：只保留中心点（对每条路径都保留中心点）
        ...
        return (np.array(impaired_windows, dtype=np.complex64),
                ...
                np.array(valid_labels_scalar, dtype=int),
                np.array(h_true_vectors, dtype=np.complex64),   # (N,5,S)
                np.array(h_est_vectors, dtype=np.complex64))    # (N,5,S)

    # =========================================================
    # ✅ 修改点 4：保存时把 (N,5,S) complex 转成 (N,10,S) float
    # =========================================================
    def _complex_to_channels(self, arr_complex):
        """
        2D: (N, L) -> (N, 2, L)
        3D: (N, K, S) -> (N, 2K, S)  (K=5 taps)
        """
        if arr_complex.ndim == 2:
            return np.stack([np.real(arr_complex), np.imag(arr_complex)], axis=1).astype(np.float32)

        if arr_complex.ndim == 3:
            N, K, S = arr_complex.shape
            out = np.zeros((N, 2 * K, S), dtype=np.float32)
            real = np.real(arr_complex).astype(np.float32)
            imag = np.imag(arr_complex).astype(np.float32)
            for k in range(K):
                out[:, 2 * k, :] = real[:, k, :]
                out[:, 2 * k + 1, :] = imag[:, k, :]
            return out
    ...
    def create_and_save_dataset(self, data_dict):
        ...
        for name, data in data_dict.items():
            ...
            if np.iscomplexobj(shuffled_data):
                data_to_save = self._complex_to_channels(shuffled_data)
            else:
    ...
    def generate(self):
        ...
        for i, k_val in enumerate(self.RICIAN_K_VALUES):
            ...
            for j in range(num_chunks):
                ...
                # ✅ 现在返回 true_h_taps: [5, num_samples]
                faded_s, true_h_taps = self.apply_tdl_channel(clean_s, k_val)

                nonlinear_s = self.apply_pa_nonlinearity(faded_s)

                nmse = np.random.uniform(*self.CHANNEL_ESTIMATION_NMSE_DB_RANGE)
                est_h_taps = self.generate_channel_estimate(true_h_taps, nmse)  # ✅ 2D 支持

                imp_w, cln_w, lbl_s, h_t_vec, h_e_vec = self.sliding_window_slicing(
                    clean_s, nonlinear_s, delay, l_chunk, true_h_taps, est_h_taps
                )
        ...
        final_data = {
            "impaired_waveforms": np.concatenate(all_imp, axis=0),  # (N, win_len) complex
            "clean_waveforms": np.concatenate(all_clean, axis=0),    # (N, win_len) complex
            "labels": np.concatenate(all_lbl, axis=0),               # (N,)
            "true_h": np.concatenate(all_h_true, axis=0),            # (N, 5, S) complex
            "estimated_h": np.concatenate(all_h_est, axis=0)         # (N, 5, S) complex (sparse center)
        }
...
def visualize_combined(data_dict, num_samples=3, save_dir="Estimate/data/tdl_data_5h/vis"):
    """
    可视化：波形 + H 轨迹
    这里为了不大改图，默认只画 “零延迟的两条 tap 相加” 作为主径等效轨迹（tap0+tap1）
    """
    import matplotlib.pyplot as plt
    ...
    h_vec = data_dict['true_h']         # (N,5,S) complex
    h_est = data_dict['estimated_h']    # (N,5,S) complex sparse
    ...
    for i, idx in enumerate(indices):
        ...
        # 主径等效：tap0 + tap1（因为前两个 norm delay 是 0）
        h = h_vec[idx][0] + h_vec[idx][1]      # (S,) complex
        h_e = h_est[idx][0] + h_est[idx][1]    # (S,) complex sparse
        ...
        axes[i, 2].plot(np.real(h), np.imag(h), 'b.-', alpha=0.6, label='True H (tap0+tap1)')
        nz = np.where(np.abs(h_e) > 1e-9)[0]
        axes[i, 2].scatter(np.real(h_e[nz]), np.imag(h_e[nz]),
                           c='r', s=100, marker='*', zorder=5, label='Center Pilot')

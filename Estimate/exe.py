...
def run_pipeline(tasks):
    ...
    print("=" * 30)
    # 格式化输出：小时和分钟取整，秒保留两位小数
    print(f"🏁 所有流程结束。总耗时: {int(hours)}时{int(minutes)}分{seconds:.2f}秒")
    print(f"📂 全部日志保存在: {os.path.abspath(log_dir)}")

if __name__ == "__main__":
    my_tasks = [
        r"F:\LJN\bishe\bishe\Estimate\rIS2B_nakagmi_resnet_adjust\test_ber.py",
        r"F:\LJN\bishe\bishe\Estimate\rIS2B_nakagmi_resnet_adjust\train_se.py",
        r"F:\LJN\bishe\bishe\Estimate\rIS2B_nakagmi_resnet_adjust\train_attention.py",
    ]

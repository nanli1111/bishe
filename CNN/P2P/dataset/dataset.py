...
class QPSKDataset(Dataset):
    def __init__(self, start_samples, end_samples, seq_len=48, n_channels=2, n_classes=4):
        ...
        self.labels = np.load(r'F:\LJN\bishe\bishe\data\rayleigh_data_all_h\labels.npy')
        ...
        self.labels = self.labels[start_samples:end_samples]  # 切割指定的样本范围
        
    def __len__(self):
        return len(self.x)

    def __getitem__(self, idx):
        return self.x[idx], self.y[idx], self.z[idx], self.labels[idx]

# 获取训练集和验证集的加载器
def get_train_QPSKdataloader(start=0, end=400000, batch_size=64, shuffle=True, val_split=0.2):
...
# 获取训练集和验证集的加载器
def get_test_QPSKdataloader(start=400000, end=500000, batch_size=64, shuffle=False):

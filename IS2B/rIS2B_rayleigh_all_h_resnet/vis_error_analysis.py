...
def visualize_specific_errors(model, is2b, tx_clean, rx_faded, h_np, snr_list, sps=16, 
                              device='cuda', save_dir='IS2B/resnet_is2b/vis_error_analysis'):
    """
    ...
    for snr_db in snr_list:
        ...
        with torch.no_grad():
            ...
            # === B. Rectified Flow Sampling (With Guidance?) ===
            # 这里建议开启 guidance > 1.0 来复现你的“过冲”问题
            # 或者设为 1.0 看无 CFG 下的累积误差
            scale = 1.0 
            x_rf = is2b.sample(y=y_tensor_noisy, h=h_tensor, guidance_scale=scale)
            # 中点采样 & 判决
...
if __name__ == "__main__":
    ...
    vis_save_dir = 'IS2B/rIS2B_rayleigh_all_h_resnet/vis_error_analysis'

    # 1. 模型
    print(f"Building TimeResNet1D on {device}...")
    model = TimeResNet1D(in_channels=4, out_channels=2, hidden_dim=128, num_blocks=8, time_emb_dim=64).to(device)
    ...
    print("Loading Data...")
    test_data = QPSKDataset(400000, 405000) # 5000个样本

...
def visualize_IS2B_restoration(model, IS2B, tx_clean, rx_faded, h_np, snr_list, sps=16, 
                               guidance_scale=1.0, # CFG 强度控制
                               device='cuda', save_dir='IS2B/IS2B_rayleigh/vis_results'):
    """
    可视化 IS2B 在 rayleigh 信道下的恢复效果（适配 x0-prediction + CFG）
    """
    ...
    for snr_db in snr_list:
        ...
        # 2. IS2B 恢复 (使用 x0-prediction 采样器)
        with torch.no_grad():
            x_rec = IS2B.sample_x0_prediction(
                y=y_tensor_noisy,
                h=h_tensor,
                eta=0.0,               # 确定性采样
        ...
        axs[0, 2].plot(x_rec_np[0, 0, :], label='IS2B 恢复', color='green', linewidth=1.5)
        axs[0, 2].set_title(f"输出：IS2B 恢复结果 (CFG w={guidance_scale})", fontsize=12)
        ...
        axs[1, 2].plot(x_rec_np[0, 1, :], label='IS2B 恢复', color='orange', linewidth=1.5)
        axs[1, 2].set_title(f"输出：IS2B 恢复结果 (CFG w={guidance_scale})", fontsize=12)
        axs[1, 2].set_xlabel('采样点索引', fontsize=10)
        axs[1, 2].legend(loc='upper right', fontsize=9)
        axs[1, 2].grid(True, linestyle=':', alpha=0.6)

        plt.suptitle(f"IS2B 信号恢复可视 (x0-Pred + CFG) @ 符号SNR {snr_db} dB\n信道 h = {h_val_str}, CFG Scale = {guidance_scale}", fontsize=16)
        plt.tight_layout()
...
if __name__ == "__main__":
    ...
    vis_save_dir = 'IS2B/IS2B_rayleigh/vis_results'
    ...
    IS2B = IS2B(model, n_steps=n_steps,

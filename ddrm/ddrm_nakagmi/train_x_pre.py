...
def train_ddrm_x0_pred(model, ddrm, train_loader, val_loader, 
    ...
    for epoch in range(1, epochs + 1):
        ...
        for clean_x, faded_y, h_est in pbar:
            ...
            y_train = faded_y.clone()
            h_train = h_expanded.clone()
            
            if mask.any():
                y_train.masked_fill_(mask, 0.0)
                h_train.masked_fill_(mask, 0.0)

            # 4. 构造网络输入 [x_t, y, h]
            net_input = torch.cat([x_t, y_train, h_train], dim=1)
        ...
        if avg_val_loss < best_val_loss:
            ...
            epochs_since_improvement = 0
            # 文件名加 _x0 标记
            torch.save(model.state_dict(), os.path.join(save_dir, f"best_model_epoch_with_n_step{ddrm.n_steps}_x0.pth"))
        else:
...
if __name__ == "__main__":
    ...
    train_ddrm_x0_pred(
        ...
        epochs=epochs,
        device=device,
        save_dir='ddrm/ddrm_nakagmi/results',
        p_uncond=0.15
    )
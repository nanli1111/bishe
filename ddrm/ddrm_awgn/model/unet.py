    ...
    def __init__(self,
        ...
        for channel, cH in zip(channels[0:-1], Hs[0:-1]):
            ...
            self.encoders.append(
                nn.Sequential(
                    UnetBlock((prev_channel, cH),  # 只使用高度 `H`
                              prev_channel,
                              channel,
                              residual=residual),
                    UnetBlock((channel, cH),
                              ...
                              channel,
                              residual=residual)))
            self.downs.append(nn.Conv1d(channel, channel, 2, 2))  # 一维卷积
            prev_channel = channel

        self.pe_mid = nn.Linear(pe_dim, prev_channel)
        channel = channels[-1]
        self.mid = nn.Sequential(
            UnetBlock((prev_channel, Hs[-1]),  # 只使用高度 `H`
                      prev_channel,
                      channel,
                      residual=residual),
            UnetBlock((channel, Hs[-1]),  # 只使用高度 `H`
                      channel,
                      channel,
                      residual=residual),
        )
        ...
        for channel, cH in zip(channels[-2::-1], Hs[-2::-1]):
            ...
            self.ups.append(nn.ConvTranspose1d(prev_channel, channel, 2, 2))  # 一维转置卷积
            self.decoders.append(
                nn.Sequential(
                    UnetBlock((channel * 2, cH),  # 只使用高度 `H`
                              channel * 2,
                              channel,
                              residual=residual),
                    UnetBlock((channel, cH),  # 只使用高度 `H`
                              channel,
                              channel,
                              residual=residual)))
        ...
        self.conv_out = nn.Conv1d(prev_channel, C, 3, 1, 1)  # 最后的输出层

...
from dataset.dataset import get_QPSKdataloader  
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % 5 == 0:
            ...
            with torch.no_grad():
                ...
                x_sample = x_sample[:8].to(device)  # 前8个信号
                ...
                # 可视化 QPSK 信号的幅度
                magnitude_noisy = torch.abs(x_noisy)
                magnitude_denoised = torch.abs(x_denoised)
                magnitude_original = torch.abs(x_sample)  # 原始信号的幅度
...
if __name__ == "__main__":
    ...
    # ===== 数据 =====
    dataloader = get_QPSKdataloader(start = 0, end = 100000, batch_size=batch_size)  # 请确保返回的数据为 (batch_size, 2, 48)

    # ===== 训练 =====
    train_ddrm(model, ddrm, dataloader, epochs=epochs, lr=lr, device=device, save_dir='ddrm/ddrm_awgn/results')

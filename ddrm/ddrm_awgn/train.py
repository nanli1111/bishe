...
def train_ddrm(model, ddrm, train_loader, val_loader, epochs=50, lr=1e-4, device='cuda', save_dir='./results', patience=10):
    ...
    for epoch in range(1, epochs + 1):
        ...
        # 如果验证损失没有改善超过 `patience` 个 epoch，则触发过拟合报警并停止训练
...
if __name__ == "__main__":
    ...
    # ===== 数据 =====
    train_loader, val_loader = get_train_QPSKdataloader(start = 0, end = 100000, batch_size=batch_size, val_split=0.2)

    # ===== 训练 =====
    train_ddrm(model, ddrm, train_loader, val_loader, epochs=epochs, lr=lr, device=device, save_dir='ddrm/ddrm_awgn/results')
